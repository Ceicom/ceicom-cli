    ...
    constructor(config) {
        ...
        this.tasks = {
            ...
            webform(str) {
                ...
                str = str.replace('[metaTag]', `${config.isCombo ? `<meta id="jsPageID" data-value="pages/${treatFileName(name)}" />` : ''}`);
                str = str.replace('[className]', config.modelPath.indexOf('.cs') > -1 ? netClass.replace('.aspx.cs', '') : netClass);
                return str;
            }
        }
    }
